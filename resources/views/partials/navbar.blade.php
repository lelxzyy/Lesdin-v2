...
<style>
  /* Hindari horizontal scroll/glitch lebar di mobile */
...
<div class="fixed inset-x-0 top-4 z-50">
  <nav class="font-poppins bg-white/95 backdrop-blur rounded-full shadow-xl px-4 sm:px-6 py-3 flex items-center w-full max-w-7xl mx-auto justify-between">
    ...
    <ul class="items-center hidden md:flex space-x-6 lg:space-x-8 text-sm font-medium text-[#3C5148]">
      ...
      @auth
        ...
        @else
          <li>
            <a href="{{ route('profile.index') }}" class="hover:text-[#1B2727] text-[#3C5148]" aria-label="Profil">
    ...
    {{-- Mobile Hamburger --}}
    <button id="menu-toggle" class="md:hidden p-2 text-[#3C5148] hover:text-[#678E4D] transition-colors relative w-10 h-10 flex-shrink-0 ml-auto" aria-label="Buka menu" aria-expanded="false" aria-controls="mobile-menu">
      <div class="relative w-7 h-7">
        ...
        <svg id="close-icon" xmlns="http://www.w3.org/2000/svg" class="lucide lucide-x absolute inset-0 w-7 h-7 opacity-0 rotate-90 transition-all duration-300 ease-in-out pointer-events-none"
          fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  {{-- Mobile Menu Panel (rounded-full) --}}
  <div
    id="mobile-menu"
    class="md:hidden absolute left-0 right-0 mx-auto mt-3 w-[92%] max-w-7xl origin-top scale-y-0 opacity-0 pointer-events-none
           bg-white/95 backdrop-blur rounded-full shadow-xl overflow-hidden transition-all duration-200 will-change-transform"
    style="transform-origin: top;"
  >
    <ul class="flex flex-col divide-y divide-gray-100 text-[#3C5148]">
      <li>
        <a href="{{ route('index') }}" class="block px-5 py-3 hover:bg-gray-50 {{ request()->is('/') ? 'text-[#678E4D] font-semibold' : '' }}">
          Beranda
        </a>
      </li>
      <li>
        <a href="{{ route('mitra.index') }}" class="block px-5 py-3 hover:bg-gray-50 {{ request()->is('mitra*') ? 'text-[#678E4D] font-semibold' : '' }}">
          Mitra
        </a>
      </li>
      <li>
        <a href="{{ route('daftar-pkl.index') }}" class="block px-5 py-3 hover:bg-gray-50 {{ request()->is('daftar-pkl*') ? 'text-[#678E4D] font-semibold' : '' }}">
          Daftar PKL
        </a>
      </li>
      ...
      @auth
        @if (request()->routeIs('profile.index'))
          <li>
            <form method="POST" action="{{ route('logout') }}" class="px-5 py-3">
              @csrf
              <button type="submit" class="w-full px-4 py-2 bg-[#3C5148] text-white rounded-full hover:bg-[#678E4D] transition-colors">
                Logout
              </button>
            </form>
          </li>
        @else
          <li>
            <a href="{{ route('profile.index') }}" class="block px-5 py-3 hover:bg-gray-50">Profil</a>
          </li>
        @endif
      @else
        <li class="px-5 py-3">
          <a href="{{ route('login') }}" class="inline-block w-full text-center px-4 py-2 bg-[#3C5148] text-white rounded-full hover:bg-[#678E4D] transition-colors">
            Login
          </a>
        </li>
      @endauth
...
{{-- Script toggle (bisa dipindah ke resources/js/app.js) --}}
<script>
  (function () {
    ...
    // Tutup kalau klik di luar panel
    document.addEventListener('click', (e) => {
      if (!isOpen) return;
      const withinNav = e.target.closest('nav');
      const withinPanel = e.target.closest('#mobile-menu');
      if (!withinNav && !withinPanel) {
        closeMenu();
        isOpen = false;
      }
    });

    // Tutup saat resize ke md ke atas
